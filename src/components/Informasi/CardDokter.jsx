...
import Image1 from "../../assets/assets/image/dokterLaki.jpg";
import * as pkg from "react-icons/fa";
import * as pkgmd from "react-icons/md";

// import Image from "next/image";

function CardDokter({ name, rating, roles, lamaKerja, id }) {
  const { FaStar } = pkg;
  const { MdWorkHistory } = pkgmd;
  return (
    <>
      <Link to={`/detaildokter/${id}`}>
          ...
          <div className="self-stretch  px-6 pt-3 pb-6 flex-col justify-start items-start flex gap-3">
            <div className="grid gap-3">
              <div className="grid gap-1">
                ...
                <p className="text-sm text-stone-500">Dokter {roles}</p>
